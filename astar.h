...
namespace alg {
	class AStar {
		public:
			static const unsigned char WALL = 'w';
		...
		public:
			 ...
			 */
			Path* run(uint32_t x1, uint32_t y1, uint32_t x2, uint32_t y2) {
				...
				while(!m_openset.is_empty()) {
					...
					if(cx == (int)x2 && cy == (int)y2) {	// great! we've reached the target position (x2,y2)
						...
						ptr->ps = (Pos*) calloc(ptr->count, sizeof(Pos));
					...
					for(nx=cx-1;nx<=cx+1;nx++) {
						for(ny=cy-1;ny<=cy+1;ny++) {
							...
							// except the wall;
							if(m_grid(nx, ny) == WALL) {
								continue;
							}
							// exclude the neighbour in the closed set	
							if(m_closedset(nx, ny)) {
								continue;
							}
